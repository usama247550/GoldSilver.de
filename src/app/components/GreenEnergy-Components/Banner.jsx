    ...
    <>
      <div className="my-3 w-[90%] md:w-[80%] mx-auto relative font-[P layfair_Display] ">
        ...
        <div className="absolute inset-0 flex flex-col justify-center gap-y-2 md:gap-y-3 ps-5 sm:ps-8 md:ps-10 pr-5 sm:pr-[45%]">
          ...
          <h1 className="text-white font-bold text-xl sm:text-3xl md:text-4xl leading-snug">{t("Green Energy & The")} <br /> {t("Solar Infrastructure Boom")}</h1>
          <p className="text-white text-xs sm:text-sm md:text-base leading-relaxed">{t("Explore how solar infrastructure, electric vehicles, and climate-tech investments are driving industrial silver demand across global markets.")}</p>
        </div>
      </div>
    </>
  );
