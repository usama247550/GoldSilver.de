  ...
  const partnerCards = [
    {
      ...
      button: "Discover More", link: process.env.NEXT_PUBLIC_AFFILIATE_SEEKINGALPHA },
    {
      logo: "AP",
      title: "Motley Fool",
      description:
      ...
      button: "Discover More", link: process.env.NEXT_PUBLIC_AFFILIATE_MOTLEYFOOL },
    {
      logo: "BV",
      ...
      button: "Discover More", link: process.env.NEXT_PUBLIC_AFFILIATE_RESEARCH },
  ];
  return (
     <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">
      {partnerCards.map((item, index) => (
        ...
        >
          ...
          <a href={item.link || '#'} target="_blank" rel="noopener noreferrer" className="mt-2 text-[#B8860B] border-2 border-[#B8860B] rounded-md text-sm font-bold py-2 hover:bg-[#B8860B] hover:text-black transition cursor-pointer block text-center">{t("Read More")}</a>
        </div>
      ))}
    </div>
  );
